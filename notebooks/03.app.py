...
# Vista para Usuarios
    ...
    # Configuración del tamaño del iframe
    powerbi_width = 600
    powerbi_height = 373.5

    # Marcado del informe embebido en Streamlit
    st.markdown(
        body=f"""
        <iframe title="Proyecto Power BI Inmobiliario" width="{powerbi_width}" height="{powerbi_height}" 
        src="https://app.powerbi.com/view?r=eyJrIjoiYzg0NDAwNzEtNmU2OC00MjU2LTg5ZmUtYzNhN2U4ZWQ1Mzk1IiwidCI6IjVlNzNkZTM1LWU4MjUtNGVkNS1iZTIyLTg4NTYzNTI3MDkxZSIsImMiOjl9&pageName=5b02787c82b37e935098" 
...
# Página: Análisis Avanzado
    ...
    def load_model():
        try:
            model = joblib.load("model.pkl")  # Ajusta el path si el modelo está en otro directorio
            return model
        except FileNotFoundError:
            st.error("No se encontró el archivo 'model.pkl'. Verifica la ruta del modelo.")
            return None
        except Exception as e:
    ...
    if model:
        ...
        def cargar_datos(tipo):
            try:
                # Selección del archivo según el tipo de operación
                data_path = "inmuebles_alquilerconcp.csv" if tipo == "Alquiler" else "inmuebles_ventaconcp.csv"
                data = pd.read_csv(data_path)
                data.columns = data.columns.str.lower().str.strip()
                ...
                # Manejo de la columna 'cp' (código postal)
                if "cp" in data.columns:
                    data["cp"] = data["cp"].astype(str).str.extract(r'(\d{5})')[0]  # Extraer códigos postales válidos
                    data = data.dropna(subset=["cp"])  # Eliminar filas con códigos postales nulos
                else:
                    st.warning("La columna 'cp' no está disponible en los datos. Algunas funcionalidades pueden estar limitadas.")
                    data["cp"] = None  # Añadir una columna vacía si no existe
        ...
        # Cargar los datos del tipo de operación
        tipo_operacion = st.selectbox("Seleccione el tipo de operación", ["Venta", "Alquiler"])
        datos = cargar_datos(tipo_operacion)

        # Extraer códigos postales únicos
        if not datos.empty and "cp" in datos.columns:
            codigos_postales = datos["cp"].dropna().unique()
            codigos_postales.sort()
        else:
            codigos_postales = []

        # Verificar si hay códigos postales disponibles
        if codigos_postales.size == 0:
            st.warning("No hay códigos postales disponibles. Revise los datos cargados.")
        ...
        codigo_postal = st.selectbox("Código Postal", codigos_postales)

        # Botón para realizar la predicción
        if st.button("Predecir Precio"):
            # Preparar los datos de entrada para el modelo
            try:
                input_data = np.array([[superficie, habitaciones, baños, int(codigo_postal)]])
                st.write("Datos de entrada para predicción:", input_data)
                ...
                st.error(f"Error con los datos de entrada: {ve}")
            except Exception as e:
                st.error(f"Error al realizar la predicción: {e}")
    else:
        st.error("El modelo no se pudo cargar correctamente. Verifica la configuración de 'model.pkl'.")
...
#Página: About Us
    ...
    integrantes = [
        {
            "nombre": "Tomás Castillo Claver",
            "linkedin": "http://linkedin.com/in/tom%C3%A1s-castillo-claver-2271002a8",
            "github": "https://github.com/tcastilloclaver",
            "imagen": "imagenes/IMG_1933.JPG" 
        },
        {
            "nombre": "Miguel González Llanera",
            "linkedin": "http://www.linkedin.com/in/miguel-gonzalez-llanera-storemanager",
            "github": "https://github.com/miguelgonzalez23",
            "imagen": "imagenes/miguel.linkeding"  
        },
        {
            "nombre": "Guillermo Pérez Toba",
            "linkedin": "https://www.linkedin.com/in/guillermo-p%C3%A9rez-toba/",
            "github": "https://github.com/GuillePT95",
            "imagen": "imagenes/guille.jpeg"  
        },
    ]
    for integrante in integrantes:
        st.image(integrante["imagen"], width=150, caption=integrante["nombre"])
